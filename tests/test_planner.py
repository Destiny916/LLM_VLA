...
class PlannerTests(unittest.TestCase):
    def test_mock_planner_returns_valid_action_names_for_binary_task(self):
        planner = OpenAICompatiblePlanner(
            client=MockClient(
                '{"visible_reasoning":"用户要求表示 01；每个动作后复位。","action_tokens":"left reset right reset"}'
            )
        )
    ...
    def test_plan_details_exposes_raw_output_reasoning_and_tokens(self):
        raw_output = '{"visible_reasoning":"用户要求右转一次；right 后复位。","action_tokens":"right reset"}'
        ...
        self.assertEqual("用户要求右转一次；right 后复位。", result.visible_reasoning)
        self.assertEqual("right reset", result.action_tokens)
    ...
    def test_planner_repairs_once_after_invalid_output(self):
        client = RecordingClient(
            [
                '{"visible_reasoning":"缺少复位。","action_tokens":"left right"}',
                '{"visible_reasoning":"已修正为每个动作后复位。","action_tokens":"left reset right reset"}',
            ]
        )
        ...
        self.assertEqual("left reset right reset", result.action_tokens)
        self.assertEqual(2, len(client.calls))
        repair_prompt = client.calls[1][-1]["content"]
        self.assertIn("left right", repair_prompt)
        self.assertIn("left", repair_prompt)
        self.assertIn("reset", repair_prompt)
        self.assertIn("重新输出", repair_prompt)

    def test_planner_rejects_non_json_model_output(self):
        planner = OpenAICompatiblePlanner(client=MockClient("left reset"))
